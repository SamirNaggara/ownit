...
export async function GET(req: NextRequest) {
  ...
  const contractAddress = req.nextUrl.searchParams.get(
    "contractAddress"
  ) as string; // Cast en string pour s'assurer du type
  ...
  if (
    ...
    return new Response(
      JSON.stringify({
        error: "SBad request: Contract address and tokenId are required.",
      }),
      {
  ...
  try {
    ...
    const tokenURI = addOrUpdateToken(
      tokenBrut.toString(),
    ...
    if (!metadataResponse.ok)
      throw new Error("Failed to fetch metadata. : " + tokenURI);
    const metadata = await metadataResponse.json();
    if (!metadata) throw new Error("Failed to parse metadata.");

    const imagePinata =
      metadata.image + "?pinataGatewayToken=" + process.env.PINATA_GATEWAY;
    return new Response(
      JSON.stringify({
        metadata,
    ...
    console.error(error);
    return new Response(
      JSON.stringify({ error: "Failed to fetch NFT metadata. : " + error }),
      {
